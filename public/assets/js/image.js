(function ($) {
    $(document).ready(function () {
      ...
      function uploadImage() {
        ...
        var uploader = $('<input type="file" accept="image/*" name="image" />')
        var images = $('.images')
        ...
        uploader.on('change', function () {
            ...
            reader.onload = function(event) {
              images.append('<div class="img thumb"  style="background-image: url(\'' + event.target.result + '\');" rel="'+ event.target.result  +'"><span>remove</span></div>')
              images.append('<input type="hidden" name="bg_img'+(i)+'" value ="'+uploader[0].files[0].name  +'" >');
              
              images.append('<input type="hidden" name="src_img'+(i++)+'" value ="'+event.target.result+'" >');  
            }
